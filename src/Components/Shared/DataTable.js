...
function DataTableHead(props) {
  ...
  let timeouts = {}
  const filterRowHandler = (headCell) => (event) => {
    clearTimeout(timeouts[headCell.id]);
    timeouts[headCell.id] = setTimeout(() =>
      headCell.filter(event.target.value),
      350
    );
  };
...
const DataTableToolbar = (props) => {
  const { numSelected, title, toggleFilterRow } = props;

  const handleFilterRowClick = (event) => {
    toggleFilterRow(event);
  };
...
function DataTable(props) {
  ...
  const { title, headCells, rows, rowKey } = props;
